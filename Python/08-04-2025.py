...
1. Abra a pasta Downloads no VSCode (file > open folder)
Você deve conseguir ver o arquivo baixado na aba de arquivos. Se não, me chame

2. Clique em terminal > new terminal

3. No terminal,  digite "python -i nome_do_arquivo.py". 
Na proxima linha, deve aparecer >>>
Se isso não acontecer, me chame

3. No python que você abriu, digite soma(1,2). O resultado deverá ser 3.
Experimente soma(4,5), soma(100,1), soma(-4,6). Não altere a função soma
que está no arquivo!
Se isso não funcionar, me chame
'''
def soma(a,b):
...
eu *sempre* quero dizer que ela recebe como argumento, não via input.
...
def runTests ():
        ...
        print("beleza, o quadrado está pronto, pode seguir com o resto")

        assert soma_dos_quadrados(2,0) == 4
        assert soma_dos_quadrados(-1,1) ==  2
        assert soma_dos_quadrados(0,0)  == 0
        assert soma_dos_quadrados(10,20) == 500
        print("beleza, tudo está pronto")
